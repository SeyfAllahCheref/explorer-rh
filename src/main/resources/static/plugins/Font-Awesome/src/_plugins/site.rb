...
module Jekyll
  ...
  class Site
    ...
    def reset_icons
      @icons = IconList.new(YAML.load_file(self.com.technods.config['icon_meta'])['icons'])
    end

    ##
    # After generation, runs a build of Font-Awesome
    def build
      system("make build", :chdir => self.com.technods.config['destination'], :out => :err)
    end

    def site_payload
      {
        "site"       => self.com.technods.config.merge({
          "time"       => self.time,
